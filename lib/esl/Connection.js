...
var Connection = module.exports = function() {
    ...
    if(eslSocket)
    {
        ...
        {
            var fCB = arguments[len - 1];
            console.log('fCB called')
            setTimeout(function(){
                fCB();
            }, 20);
        }
    ...
    this.len = len;
    this.arguments0 = arguments[0]
    this.arguments1 = arguments[1]
    this.arguments2 = arguments[2]
...
Connection.prototype.bgapi = bgapi;
Connection.prototype.api   = bgapi;

function bgapi(command, args, jobid, cb) {
    if(typeof args === 'function') {
        cb = args;
        args = '';
        jobid = null;
    ...
    if( eslSocket.isReconnect === true ) {

        cb({"body":"-ERR MEDIA DISCONNECT"});
        return;
    }
...
Connection.prototype.auth = function(cb) {
    ...
    self.sendRecv('auth ' + self.password, function(evt) {
        if(evt.getHeader('Modesl-Reply-OK') === 'accepted') {
            ...
            if( this.listeningEvents.length == 0  ) {
            ...
            {
                console.log("Event Connected again and going to re subscribe...");
                console.log(this.listeningEvents);
                this.subscribe( this.listeningEvents  );
                this.isReconnect = false;
            }
        } else {
            self.authed = false;
...
Connection.prototype.subscribe = function(events, cb, cbStore) {

    // if( this.isReconnect === true && typeof this.eventsThatNeedstoSetAgain == "undefined" ) {

    //     console.log('Not Needed subscribe', events)
    //     return;
    // } 

    events = events || 'all';

    // this.eventsThatNeedstoSetAgain = events;
    console.log('subscribe start', events)
...
Connection.prototype._onError = function(err) {
    // this.emit('error', err);
    var _this = this;
    if( isInitalFirstTime === true ) {
        isInitalFirstTime = false;
        // this.emit('error', err);
    }
...
Connection.prototype.initSocket = function(isReconnect) {
    console.log("INIT socket with isReconnecting ", isReconnect)
    var self = this;
    var len = this.len;
    ...
    if(len === 3 || len === 4) { //3 (host, port, password); 4 (host, port, password, callback)
        ...
        //save password
        this.password = this.arguments2;

        this.isReconnect = isReconnect;
        //connect to ESL Socket
        this.socket = net.connect({
            port: this.arguments1,
            host: this.arguments0
        }, this._onConnect.bind(this));

        this.socket.on('error', this._onError.bind(this));
        this.socket.on('end',   this._onError.bind(this));
    }
    ...
    if( isInitalFirstTime === true ) {
        ...
        console.log("Listening first time")
